...
def save_to_json(data):
    current_time = datetime.datetime.now().strftime("%m%d_%H_%M")
...
def parse(html):
    ...
    for repoNode in repoLst:
        try:
            # Description
            desc = repoNode.xpath('div/div[1]/div/span/text()')
            repoDesc = desc[0].strip() if desc else ""
            # Stars
            star = repoNode.xpath('div/div[1]/ul/li[2]/a/span/text()')
            repoStar = star[0].strip() if star else ""
            # Clone src
            repoSrc = repoNode.xpath('div/div[1]/h3/div/div[2]/a/@href')[0].strip()
            repoSrc = "https://github.com" + repoSrc + "/archive/refs/heads/master.zip"
            print(f"Parsing >> {repoSrc}")
            # Updated time
            spanNode = repoNode.xpath('div/div[1]/ul/li[3]/span')
            if spanNode: updatedTxt = "".join(spanNode[0].xpath('.//text()'))
            repoUpdated = updatedTxt.replace("Updated ", "")
        except:
            continue
        ...
        result.append({"info": repoDesc,
                       ...
                       "updated": repoUpdated})
        time.sleep(0.08)
    print(f"Got {len(repoLst)} Repos and successfully parsed {len(result)} of them...")
    return result
...
if __name__ == '__main__':
    ...
    for i in range(startPage, endPage+1):
        print("\n" + f"Parsing >> Page NO.{i}".center(80, "-"))
        # url = f"https://github.com/search?q=language%3AVerilog+&type=repositories&p={i}"
        url = f"https://github.com/search?q=language%3AVerilog+&type=repositories&s=updated&o=desc&p={i}"
        html = req(url)
        result = parse(html)
        allResult.extend(result)
        time.sleep(0.3)
    print(f"Total saved >> {len(allResult)}")
